...
qsiprep_bp = Blueprint('qsiprep', __name__)
...
def build_qsiprep(bids_abs, output_abs, work_abs, license_abs, subject_id):
    command = [
...
@qsiprep_bp.route('/api/run_qsiprep', methods=['POST'])
def run_qsiprep():
    ...
    # Check DWI files exist
    dwi_dir = os.path.join(bids_dir, f'sub-{subject_id}', 'dwi')
    if not os.path.exists(dwi_dir) or not any(
        f.endswith(('.nii', '.nii.gz')) for f in os.listdir(dwi_dir)
    ):
        return jsonify({'status': 'error', 'message': 'No DWI files found for this subject'}), 404
    ...
    image, command, volumes = build_qsiprep(
    ...
    return jsonify({'status': 'success', 'message': 'QSIPrep started', 'container_id': container_id})


@qsiprep_bp.route('/api/qsiprep_status/<folder_name>', methods=['GET'])
def qsiprep_status(folder_name):
    output_dir     = os.path.join(folder_name, f'preproc_{PIPELINE_ID}')
    container_name = f'preproc_{PIPELINE_ID}_{folder_name}'
...
@qsiprep_bp.route('/api/qsiprep_results/<folder_name>', methods=['GET'])
def qsiprep_results(folder_name):
    output_dir = os.path.join(folder_name, f'preproc_{PIPELINE_ID}')
    return jsonify({
        'status':  'success',
...
@qsiprep_bp.route('/api/qsiprep_report/<folder_name>/<path:filename>')
def qsiprep_report(folder_name, filename):
    output_dir = os.path.abspath(os.path.join(folder_name, f'preproc_{PIPELINE_ID}'))
    return send_from_directory(output_dir, filename)


@qsiprep_bp.route('/api/qsiprep_logs/<folder_name>', methods=['GET'])
def qsiprep_logs(folder_name):
    container_name = f'preproc_{PIPELINE_ID}_{folder_name}'
